<?php /*a:1:{s:68:"/home/wwwroot/demo70.zhigou888.com/app/super/view/autoreg/index.html";i:1764755762;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
    ...
    <![endif]-->
    <title>自动注册/登录管理</title>
   <script type="text/javascript" src="/static/lib/jquery/1.9.1/jquery.min.js"></script>

</head>
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 自动注册/登录 <span class="c-gray en">&gt;</span> 配置列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
            ...
            <tbody>
               <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$f): $mod = ($i % 2 );++$i;?>
                    <tr class="text-c">
                        ...
                        <td><?php echo htmlentities($f['auth_code']); ?></td>
        ...
        <div class="modal-dialog">
            <div class="modal-content radius">
                <div class="modal-header">
                    <h3 class="modal-title">编辑自动注册/登录配置信息</h3> 
                ...
                <div class="modal-body">
                    ...
                    <div class="row cl">
						<label class="form-label col-xs-4 col-sm-3">来源URL：</label>
                   ...
                   <div class="row cl">
						<label class="form-label col-xs-4 col-sm-3">校验码：</label>
						<div class="formControls col-xs-8 col-sm-9">
							<input type="text" maxlength="20" class="input-text radius"  id="edit_authcode"  name="authcode" autocomplete="off"
                              placeholder="校验码(3-20位数字+字母，必填)">
						</div>
					</div>
                </div>
                <div class="modal-footer">
                    <button type="button" onClick="checkEditData(this);" class="btn btn-primary radius">保存</button>
...
function checkEditData(obj){
   ...
   let authCode = $("#edit_authcode").val();
    ...
    if($("#edit_url").val().length==0 || 
     ...
     if (authCode.length<3 && authCode.length>20){
        ret = false;
        $("#edit_authcode").focus();
        alert("请确认校验码是否为3-10位字符！");
        return;
    }
...
function submitUpdate(obj){
   ...
   //  alert(data);
     $.ajax({
        url: "<?php echo url('/super_updateAutoreg'); ?>",
        type: 'POST',
        data: data,
        async: false,      
        success: function (data) {
            ...
            if(data.err == 0){
                layer.msg('提交成功！');  
                $(obj).next().click(); 
                setTimeout("window.location.href = '<?php echo url('/super_autoReg'); ?>'",600)
            }else{
                layer.msg(data.msg);
            }
        },
...
function edit(obj,id){
      ...
      $.ajax({
                type:'get',
                url:"<?php echo url('/super_autoRegEdit'); ?>",
                data:{
                    id:id
                },
                success:function (data) {
                    if(data.err == 0)
                    {
                        ...
                        $("#edit_authcode").val(gameObj.auth_code)
                    }
                    else
                    {
                       // layer.alert(data.msg);
