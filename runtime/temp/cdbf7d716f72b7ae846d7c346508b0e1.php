<?php /*a:1:{s:92:"/home/wwwroot/demo70.zhigou888.com/app/super/view/waistcoatmember/waistcoat_member_list.html";i:1764755762;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
        ...
        <div class="modal-dialog">
            <div class="modal-content radius">
                <div class="modal-header">
                    <h3 class="modal-title">添加马甲用户</h3>
                ...
                <form method="POST" id="formAdd" class="form form-horizontal">
                    <div class="modal-body">
                        ...
                        <!--</div>-->
                         <div class="row cl">
                            <label class="form-label col-xs-4 col-sm-3">马甲用户名集合：</label>
                            <div class="formControls col-xs-8 col-sm-9">
                                <textarea id="waistcoatText" rows="15" cols="50"></textarea>
                                <button type="button" onClick="createWaistcoat(this);" class="btn btn-primary radius">系统生成</button>
                            </div>
                        ...
                        <!--</div>-->
                        <div class="row cl">
                            <label class="form-label col-xs-4 col-sm-3">默认密码(必须)：</label>
                            <div class="formControls col-xs-8 col-sm-9">
                                <input type="text" class="input-text radius" id="waistcoatPassword" name="password"
                    ...
                    <div class="modal-footer">
                        <button type="button" onClick="checkWaistcoatData(this);" class="btn btn-primary radius">确定</button>
...
<script type="text/javascript">
    ...
    //验证数据
    function createWaistcoat(obj) {
      //  alert("123");
        document.getElementById("waistcoatText").innerHTML="♚ 伪装面具,4心底故人,一醉方休";
    }
    
    //验证马甲用户数据
    function checkWaistcoatData(obj) {
        var msg = $("#waistcoatText").val();  //textarea  文本框输入的内容
        var passwordStr = $("#waistcoatPassword").val(); //密码
        ...
        var nicknameArr = [];   //定义一个数组,用来存msg分割好的内容
        nicknameArr = msg.split(",");
        
        for(let i=0; i<nicknameArr.length; i++){
          nicknameArr[i] = nicknameArr[i].replace(/[\r\n]/g,"");
          nicknameArr[i] = $.trim(nicknameArr[i]);
　       　if(nicknameArr[i] == ""){
　　　       　nicknameArr.remove(nicknameArr[i]);
　    　    }
        }
        ...
        var accountText = "jhgfxcuivbonmnwjubyctevbioqudiyvcxbnmqkwdjnbhcvghijokqmwjnehgvwcbxainmyvtyrd657g8h1i8s7wtfdmui75654xcviybuoi98hin";
        var accountAry = [];
        accountAry = accountText.split("");
        for(var j=0;j<nicknameArr.length;j++){
            for(var i=Math.floor(Math.random()*5);i<5;i++)
         {
            accountStr+=accountAry[Math.floor(Math.random()*50)];
         }
        for(var i=Math.floor(Math.random()*5);i<12;i++)
         {
            accountStr+=Math.floor(Math.random()*10);
         }
         accountStr="is_customer_service=0&username="+accountStr+"&nickName="+nicknameArr[j]+"&password="+passwordStr+"&isWaistcoat=1";
         account[j] = accountStr;
         accountStr = "";
        }
       ...
       layer.msg('提交成功！');
       $(obj).next().click(); 
       setTimeout("window.location.href = '<?php echo url('/super_waistcoatMemberList'); ?>'",1000);
    ...
    function checkData(account) {
        ...
        //     return;
        // }
        if (ret) submitSave(account);
    }


    //提交数据
    function submitSave(account) {
